...
function updateLibraryInfo(stats, uniqueJoistCount, avgInstantiationTime) {
  ...
  const librarySpecificationsElement = document.getElementById("library-specifications");
  ...
  if (variantsElement) variantsElement.textContent = `Variants: ${stats.totalVariants}`;
  if (libraryMemoryElement) libraryMemoryElement.textContent = `Library Size: ${stats.memoryFootprint}`;
  if (librarySpecsElement) librarySpecsElement.textContent = `Length Range: ${stats.lengthRange}`;
  if (libraryDepthsElement) libraryDepthsElement.textContent = `Depth Range: ${stats.depthRange}`;
  if (libraryPatternsElement) libraryPatternsElement.textContent = `Web Patterns: ${stats.webPatterns}`;
  if (libraryRevisionsElement) libraryRevisionsElement.textContent = `Design Revisions: ${stats.designRevisions}`;
  if (librarySpecificationsElement) librarySpecificationsElement.textContent = `Specifications: ${stats.specifications}`;
  if (uniqueJoistsElement) uniqueJoistsElement.textContent = `Unique Instances: ${uniqueJoistCount}`;
  if (instantiationElement) instantiationElement.textContent = `Avg Instantiation: ${avgInstantiationTime}ms`;
}
...
function toggleFloorVisibility(floorIndex, visible) {
  if (joistsByFloor[floorIndex]) {
    ...
    console.log(`Big Kahuna Floor ${floorIndex + 1} (${joistCount} joists) visibility toggled to ${visible} in ${(endTime - startTime).toFixed(2)}ms`);
  }
}
...
function simulateDesignRun() {
  ...
  // Process joists in larger batches for Big Kahuna efficiency
  function processBatch() {
    ...
    if (processedCount < totalJoists) {
      ...
      console.log(`Big Kahuna design analysis complete: ${totalJoists.toLocaleString()} joists analyzed in ${analysisTime}ms`);
...
async function main() {
  ...
  try {
    if (progressText) progressText.innerText = "🌺 THE BIG KAHUNA: Initializing OpenCascade...";
    const oc = await initOpenCascade();

    if (progressText) progressText.innerText = "🌺 Pre-tessellating 10000-variant joist library...";
    if (timeInfo) timeInfo.innerText = "Building the ultimate joist library (one-time cost)";
    ...
    if (progressText) progressText.innerText = "🌺 THE BIG KAHUNA: Instantiating 10,000 joists...";
    if (timeInfo) timeInfo.innerText = `Library loaded in ${libraryTime}s. Now for the ultimate test...`;
    ...
    const result = await createBigKahunaJoists(scene, (message, current, total, uniqueCount, avgTime) => {
      ...
      if (timeInfo) timeInfo.innerText = `Library: ${libraryTime}s | Instantiation: ${elapsed}s | Speed: ${joistsPerSecond} joists/sec`;
    ...
    console.log(`- Used ${result.uniqueVariantsUsed} unique variants from 10000-variant library`);
    ...
    if (progressText) progressText.innerText = `🌺 BIG KAHUNA COMPLETE! ${result.meshes.length.toLocaleString()} joists (${result.uniqueVariantsUsed} unique variants)`;
    if (timeInfo) timeInfo.innerText = `Library: ${libraryTime}s | Instantiation: ${instantiationTime}s | Total: ${totalTime}s | Speed: ${joistsPerSecond} joists/sec`;
    ...
    const elapsed = ((performance.now() - startTime) / 1000).toFixed(1);
    if (progressText) progressText.innerText = "🌺 Big Kahuna encountered an error. Check console for details.";
